...
type TaskForm = { clientName: string; clientAddress: string; clientPhone: string; notes: string; documents: string[]; customDocument: string; priority: "normal" | "high" | "urgent"; dueAt: string; destinationLat?: number; destinationLon?: number };
...
export default function DashboardPage() {
  ...
  async function openEditTask(task: DashboardTask) {
    ...
    setTaskForm({ clientName: task.clientName, clientAddress: task.clientAddress, clientPhone: task.clientPhone ?? "", notes: task.notes ?? "", documents: task.documents?.map((document) => document.name) ?? [], customDocument: "", priority: task.priority === "high" || task.priority === "urgent" ? task.priority : "normal", dueAt: task.dueAt ? new Date(task.dueAt).toISOString().slice(0,16) : "", destinationLat: task.destinationLat, destinationLon: task.destinationLon });
    try { const data = await api<{ documentTypes: Array<{ name: string }> }>("/api/document-types"); setDocumentTypes(data.documentTypes.map((item) => item.name)); }
    catch { setDocumentTypes([]); }
  }
      ...
      {taskRunner && (
        <div className="fixed inset-0 z-[1400] flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-4">
          <form onSubmit={saveTask} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-[28px] bg-surface p-5 shadow-2xl sm:rounded-[28px] sm:p-6">
            ...
            <div className="mb-3 grid grid-cols-1 gap-3 sm:grid-cols-2"><label className="text-sm">Priority<select className="mt-1 w-full rounded-xl border border-[#777680] bg-transparent px-3 py-2" value={taskForm.priority} onChange={(e) => setTaskForm({ ...taskForm, priority: e.target.value as TaskForm["priority"] })}><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></label><label className="text-sm">Due time <span className="text-on-surface-variant">(optional)</span><input type="datetime-local" className="mt-1 w-full rounded-xl border border-[#777680] bg-transparent px-3 py-2" value={taskForm.dueAt} onChange={(e) => setTaskForm({ ...taskForm, dueAt: e.target.value })} /></label></div>
