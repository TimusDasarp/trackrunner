...
| 6. Mobile release readiness | In progress | Restored the Expo 56 courier client as a normal repository folder (not a submodule), replaced emulator-only HTTP configuration with required `EXPO_PUBLIC_API_BASE_URL`, removed the inherited EAS project link, added a staging run guide, and verified TypeScript plus Expo configuration locally. The client has SecureStore session storage, SQLite event queue, server acknowledgement before deletion, and foreground/background location configuration. A physical-device staging test, privacy flow, crash reporting, and signed internal build remain. |
